...
from bmi_core import bmi_category, auto_group, group_display_name, Config
...
class BMIVisualizer:
    ...
    def _create_guidance_chart(self, ax, bmi: float, age: int, gender: str, group: str, lang: str):
        ...
        colors = ['lightgreen', 'blue' if healthy_min <= current_weight <= healthy_max else 'orange', 'lightgreen']
        ...
        if current_weight < healthy_min:
            recommendation = "Consider healthy weight gain" if lang == "en" else "Рекомендуется здоровый набор веса"
        elif current_weight > healthy_max:
            recommendation = "Consider healthy weight loss" if lang == "en" else "Рекомендуется здоровое снижение веса"
        else:
            recommendation = "Maintain current weight" if lang == "en" else "Поддерживайте текущий вес"
