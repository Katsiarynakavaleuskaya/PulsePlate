...
module SemanticPolicy
  ...
  METADATA_MEDICAL_CLAIMS = /
    ...
    \b(?:ИМТ|медицин(?:а|ский|ская|ские|ских)|врач(?:а|ей|ом)?|диагноз(?:а|ом|е|ы)?|пациент(?:а|ов)?|терап(?:ия|ии|ию|ией)|клиничес[а-яё]*|лекарств[а-яё]*|препарат[а-яё]*|таблет[а-яё]*|диетолог[а-яё]*|нутрициолог[а-яё]*|эксперт[а-яё]*\s+по\s+питанию)\b|
  ...
  STORE_TRUTH_CLAIMS = [
    ...
    /\b(?:(?:оформ(?:ите|ить|ление)|куп(?:ите|ить)|подключ(?:ите|ить)|условия|стоимост[а-яё]*|цен[а-яё]*|тариф[а-яё]*|оплат[а-яё]*|доступ)\s+(?:[а-яё]+\s+){0,3}подписк[а-яё]*|подписк[а-яё]*\s+(?:для\s+доступа|на\s+(?:месяц|год|план)|за\s+storekit|стоимост[а-яё]*|цен[а-яё]*|условия))\b/i,
    /\b(?:(?:скидк[а-яё]*|промокод[а-яё]*|купон[а-яё]*)\s+(?:\d|на\s+(?:годов(?:ой|ую)|месячн(?:ый|ую)|план|подписк[а-яё]*)|для\s+доступа|в\s+(?:app\s+store|storekit))|(?:получ(?:ите|ить)|оформ(?:ите|ить)|использ(?:уйте|овать)|активиру(?:йте|й)|условия|цен[а-яё]*|тариф[а-яё]*)\s+(?:[а-яё]+\s+){0,3}(?:скидк[а-яё]*|промокод[а-яё]*|купон[а-яё]*))/i,
