    ...
    var body: some View {
        Group {
            if let animation = animation {
                ...
                Image("FitChef", bundle: .module)
                    .resizable()
                    .scaledToFit()
            }
        }
        .onAppear { loadAnimation() }
        .onChange(of: animationName) { _ in loadAnimation() }
    }

    private func loadAnimation() {
        // 1) Try to load from SPM resources first (Bundle.module)
        if let anim = LottieAnimation.named(animationName, bundle: .module) {
            animation = anim
            print("✅ Lottie (module) loaded: \(animationName)")
            return
        }
        // 2) Fallback to main bundle (if app isn’t packaged as SPM target)
        if let anim = LottieAnimation.named(animationName, bundle: .main) {
            animation = anim
            print("✅ Lottie (main) loaded: \(animationName)")
            return
        }
        // 3) Not found
        animation = nil
