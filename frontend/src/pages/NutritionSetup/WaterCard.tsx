...
export default function WaterCard({ liters }: WaterCardProps) {
  ...
  const displayLiters = Number.isFinite(liters) && liters > 0 ? liters.toFixed(1) : '2.0'; // fallback value
    ...
    <div className="bg-white rounded-2xl p-6 shadow-sm">
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-muted text-sm mb-2">{t('nutrition.water.subtitle')}</p>
      ...
      <div className="mt-4 p-3 bg-blue-50 rounded-lg">
        <p className="text-xs text-blue-800">{t('nutrition.water.tip')}</p>
      </div>
    </div>
  );
}
