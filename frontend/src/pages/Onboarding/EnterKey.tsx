...
export default function EnterKey() {
  ...
  const handleSave = () => {
    ...
    try {
      ...
      let message = t("onboarding.enterKey.errorGeneric", "Invalid API key");
      if (error instanceof AuthError) {
        if (error.code === "API_KEY_TOO_SHORT") {
          message = t("auth.apiKey.tooShort");
        } else if (error.code === "API_KEY_INVALID_FORMAT") {
          message = t("auth.apiKey.invalidFormat");
        }
      }
      toast.error(message);
    }
  ...
  const handleClear = () => {
    const hadKey = Boolean(value.trim());
    auth.clearApiKey();
    setValue("");
    if (hadKey) {
  ...
  return (
    <div className="max-w-md mx-auto p-6 space-y-4">
      <header>
        ...
        <p className="text-sm text-gray-400">{t("onboarding.enterKey.description")}</p>
      </header>
      ...
      <p className="text-xs text-gray-500">{t("onboarding.enterKey.changesNote")}</p>
    </div>
  );
}
