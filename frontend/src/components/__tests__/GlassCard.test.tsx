...
import type { GlassCardTone, GlassCardPadding } from "../GlassCard";
...
describe("GlassCard", () => {
  ...
  const toneClassMap: Record<GlassCardTone, string[]> = {
  ...
  Object.entries(toneClassMap).forEach(([tone, expectedClasses]) => {
    it(`applies tone styles for tone="${tone}"`, () => {
      render(
        <GlassCard tone={tone as GlassCardTone}>
          <span>content</span>
        </GlassCard>
      );
  ...
  const paddingClassMap: Record<GlassCardPadding, string | null> = {
    none: null,
    sm: "p-3",
    md: "p-4",
    lg: "p-6",
  ...
  Object.entries(paddingClassMap).forEach(([padding, expectedClass]) => {
    it(`applies padding styles for padding="${padding}"`, () => {
      render(
        <GlassCard padding={padding as GlassCardPadding}>
          <span data-testid="inner" />
        </GlassCard>
      );
