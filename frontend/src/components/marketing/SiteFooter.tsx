      ...
      <div className="ppm-footer">
        <div className="ppm-footer-copy">
          ...
          <p className="ppm-description" style={{ fontSize: '0.95rem' }}>
            A calm, premium wellness control panel built around real product surfaces, honest
            scope, and bounded guidance.
          </p>
          <p className="ppm-footer-note" style={{ marginTop: '0.75rem' }}>Product-first clarity. Quiet confidence.</p>
        </div>
            ...
            <div key={group.title}>
              ...
              <div className="ppm-footer-link-list">
                {group.links.map((link) =>
                  'to' in link ? (
                    <Link key={link.label} className="ppm-footer-link" to={link.to}>
                      {link.label}
                    </Link>
                  ) : (
                    <a key={link.label} className="ppm-footer-link" href={link.href}>
                      {link.label}
                    </a>
                  )
                )}
