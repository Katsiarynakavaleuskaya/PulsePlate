...
// Base URL for MSW tests - using absolute URLs for Node.js/jsdom compatibility
const API_BASE_URL = "http://localhost";

test("msw /api/purchase responds ok", async () => {
  const res = await fetch(`${API_BASE_URL}/api/purchase`, { method: "POST" });
...
test("msw /api/purchase handles server error", async () => {
  const res = await fetch(`${API_BASE_URL}/api/purchase?error=server`, { method: "POST" });
...
test("msw /api/purchase handles payment error", async () => {
  const res = await fetch(`${API_BASE_URL}/api/purchase?error=payment`, { method: "POST" });
...
test("msw /api/restore responds ok", async () => {
  const res = await fetch(`${API_BASE_URL}/api/restore`, { method: "POST" });
  const json = await res.json();
  expect(json.status).toBe("ok");
...
test("msw /api/restore handles server error", async () => {
  const res = await fetch(`${API_BASE_URL}/api/restore?error=server`, { method: "POST" });
...
test("msw /api/restore handles not found error", async () => {
  const res = await fetch(`${API_BASE_URL}/api/restore?error=not_found`, { method: "POST" });
