...
test("msw /api/purchase handles server error", async () => {
  const res = await fetch("/api/purchase?error=server", { method: "POST" });
...
test("msw /api/purchase handles payment error", async () => {
  const res = await fetch("/api/purchase?error=payment", { method: "POST" });
...
test("msw /api/restore handles server error", async () => {
  const res = await fetch("/api/restore?error=server", { method: "POST" });
...
test("msw /api/restore handles not found error", async () => {
  const res = await fetch("/api/restore?error=not_found", { method: "POST" });
