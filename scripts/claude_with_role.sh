...
# To re-enable: restore original script from git history

echo "⚠️  Claude CLI временно отключен (аккаунт в бане)"
echo "Скрипт claude_with_role.sh деактивирован для предотвращения использования GPU"
echo ""
echo "Для повторной активации после восстановления аккаунта:"
echo "  git checkout scripts/claude_with_role.sh"
exit 1

# Original script code (disabled):
