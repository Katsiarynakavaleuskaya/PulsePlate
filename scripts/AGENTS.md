...
- PR-2 creative-code generation gate artifacts stay local under
  ...
  sanitized `generation_receipt.json`. It must not change role order, mark role
  passes complete, create/write branches, push/open PRs, resolve review threads,
  edit fixed mappings, claim readiness, promote candidates, call providers,
  call product runtime, modify workflows, use semantic cache, write graph truth,
  or mutate GitHub App / Slack settings.
