...
POST_OPEN_REVIEW_GUIDANCE = (
    ...
    "`pulseplate-pr-review` as the local self-review; it does not itself satisfy "
    "the trusted review-evidence slot. Satisfy the final review-evidence slot with "
    "exactly one of a completed trusted exact-head GitHub Codex Connector review "
    "or an unedited trusted terminal Connector rate/usage-limit receipt with "
    "`review_claim=none`; the terminal unavailable path requires no retry or "
    "substitute review. Any material fix invalidates the freeze and returns to "
    "fixes, local/current-head gates, freeze, and review evidence. Then run "
    "`pr_review_closeout.py prepare-final-security --repo <owner/name> "
    "--pr-number <N>` with the matching `--review-ref <exact-head-review-URL>` "
    "or `--review-source-unavailable-ref <trusted-terminal-comment-URL>` and have the "
    ...
    "cannot prove global cross-machine request consumption. Every terminal "
    "outcome (`completed`, `timeout`, `safety_block`, or `incomplete`) consumes "
    "the operational request and must be recorded with "
