...
PHILOSOPHY_ADMISSION_FORBIDDEN_PATTERNS = (
    (
        ...
        re.compile(
            ...
            r"(?:(?:will|shall|can|may|must|could|would|should|might) "
            r"(?:re[- ]?open|open(?: up)?|unlock|activate|enable)|"
            r"(?:(?:has|have|had|needs?|need) to "
            r"(?:re[- ]?open|open(?: up)?|unlock|activate|enable))|"
            r"(?:has|had) (?:re[- ]?opened|opened(?: up)?|unlocked|activated|enabled)|"
            ...
            r"|\b(?:the )?(?:global|semantic[- ]cache) gate "
            r"(?:is|was|has been|had been) (?:now )?being "
            r"(?:opened|re[- ]?opened|unlocked|activated|enabled)"
            r"(?: for (?:philosophy pr-1|pr-1|philosophy admission))?\b"
            r"|\b(?:the )?(?:global|semantic[- ]cache) gate "
            r"(?:has|have|had) to be "
            r"(?:opened|re[- ]?opened|unlocked|activated|enabled)"
            r"(?: for (?:philosophy pr-1|pr-1|philosophy admission))?\b"
            r"|\b(?:the )?(?:global|semantic[- ]cache) gate "
            ...
            r"|\b(?:the )?(?:global|semantic[- ]cache) gate "
            r"(?:can|may|must|could|would|should|might) be opened "
            r"for philosophy admission\b"
            r"|\b(?:the )?(?:global|semantic[- ]cache) gate "
            r"(?:can|may|must|could|would|should|might) be "
            ...
            r"|\b(?:the )?(?:(?:global )?semantic[- ]cache|global) gate "
            r"(?:will|shall|must|could|would|should|might) (?:be )?"
            r"(?:open|opened|re[- ]?opened|unlocked|activated|enabled) "
            r"(?:in|during|with|by) (?:philosophy )?pr-1\b"
            r"|\b(?:philosophy )?pr-1(?: admission)? "
            r"(?:can|may|must|could|would|should|might) "
            r"(?:open|unlock|activate|enable) "
            r"(?:the )?(?:global|semantic[- ]cache) gate\b"
            r"|\b(?:philosophy admission|philosophy pr-1 admission|"
            r"philosophy pr-1|pr-1 admission|pr-1|the admission contract|"
            r"admission contract) "
            r"(?:is|are|was|were|has been|have been) required to "
            r"(?:re[- ]?open|open(?: up)?|unlock|activate|enable) "
            r"(?:the )?(?:global|semantic[- ]cache) gate\b"
            r"|\bphilosophy admission (?:can|may|must|could|would|should|might) "
            r"(?:open|unlock|activate|enable) "
            r"(?:the )?(?:global|semantic[- ]cache) gate\b"
        ),
    ),
    ...
    (
        ...
        re.compile(
            ...
            r"|\b(?:philosophy admission|philosophy pr-1|pr-1) "
            r"(?:(?:will|shall|can|may|must|could|would|should|might) "
            r"(?:add|import|use|wire)|(?:adds?|imports?|uses?|wires?)|"
            r"(?:has|had) (?:added|imported|used|wired)) "
            r"redis (?:imports?|clients?|probes?)\b"
            r"|\b(?:philosophy admission|philosophy pr-1|pr-1) "
            r"(?:has|had|includes?|included|contains?|contained) "
            r"redis (?:imports?|clients?|probes?)"
            r"(?: for philosophy admission)?\b"
            r"|\b(?:philosophy admission|philosophy pr-1|pr-1) "
            r"(?:(?:will|shall|can|may|must|could|would|should|might) "
            r"(?:add|import|use|wire)|(?:adds?|imports?|uses?|wires?)|"
            r"(?:has|had) (?:added|imported|used|wired)) redis\b"
            r"|\b(?:philosophy admission|philosophy pr-1|pr-1) "
            r"(?:(?:is|are|was|were|has been|have been) )?"
    ...
    (
        ...
        re.compile(
            ...
            r"(?:add|import|use|wire)|(?:adds?|imports?|uses?|wires?)|"
            r"(?:has|had) (?:added|imported|used|wired)) "
            r"gptcache (?:imports?|clients?|probes?)\b"
            r"|\b(?:philosophy admission|philosophy pr-1|pr-1) "
            r"(?:has|had|includes?|included|contains?|contained) "
            ...
            r"(?:(?:will|shall|can|may|must|could|would|should|might) "
            r"(?:add|import|use|wire)|(?:adds?|imports?|uses?|wires?)|"
            r"(?:has|had) (?:added|imported|used|wired)) gptcache\b"
            r"|\b(?:philosophy admission|philosophy pr-1|pr-1) "
            r"(?:(?:is|are|was|were|has been|have been) )?"
    ...
    (
        ...
        re.compile(
            ...
            r"(?:redis |gptcache )?connection strings?\b"
            r"|\b(?:philosophy admission|philosophy pr-1|pr-1) "
            r"(?:connects to|connected to|(?:has|had) connected to) (?:redis|gptcache) "
            r"for philosophy admission\b"
            r"|\b(?:philosophy admission|philosophy pr-1|pr-1) "
            r"(?:opens?|opened|(?:has|had) opened) (?:a )?(?:redis|gptcache) "
            r"connections? for philosophy admission\b"
            r"|\b(?:philosophy admission|philosophy pr-1|pr-1) "
            r"(?:has|have|had) (?:a )?(?:redis|gptcache) connection strings? "
            r"for philosophy admission\b"
            r"|\b(?:redis |gptcache )?connection strings? "
            r"(?:(?:are|is|was|were|has been|have been) )?"
    ...
    (
        ...
        re.compile(
            ...
            r"|\b(?:philosophy admission|philosophy pr-1|pr-1) "
            r"(?:(?:will|shall|can|may|must|could|would|should|might) "
            r"(?:add|use|wire|call)|adds?|uses?|wires?|calls?|"
            r"(?:has|had) (?:added|used|wired|called)) "
            r"(?:providers?|provider calls?|storage|semantic[- ]cache storage|cache storage)"
            r"(?: for philosophy admission)?\b"
            r"|\b(?:philosophy admission|philosophy pr-1|pr-1) "
            r"(?:(?:will|shall|can|may|must|could|would|should|might) "
            r"call|calls?|(?:has|had) called) "
            r"(?:openai|anthropic|google|azure|llm|model) providers?"
            r"(?: for philosophy admission)?\b"
            r"|\b(?:philosophy admission|philosophy pr-1|pr-1) "
            r"(?:(?:will|shall|can|may|must|could|would|should|might) "
            r"(?:add|use|wire)|adds?|uses?|wires?|(?:has|had) (?:added|used|wired)) "
            r"(?:openai|anthropic|google|azure|llm|model) providers?"
            r"(?: for philosophy admission)?\b"
        ),
    ),
    (
        ...
        re.compile(
            ...
            r"|\b(?:philosophy admission|philosophy pr-1|pr-1) "
            r"(?:(?:will|shall|can|may|must|could|would|should|might) "
            r"(?:read from|write to|store)|reads? from|writes? to|stores?|"
            r"(?:is|are|was|were|has been|have been) "
            r"(?:reading from|writing to|storing)|"
            r"(?:has|had) (?:read from|written to|stored))"
            r"(?: (?:semantic[- ]cache|cache))?"
            r"(?: entries?)?\b"
            r"|\b(?:philosophy admission|philosophy pr-1|pr-1) "
            r"(?:(?:will|shall|can|may|must|could|would|should|might) "
            r"(?:read|write)|reads?|writes?|"
            r"(?:is|are|was|were|has been|have been) (?:reading|writing)|"
            r"read|wrote|stored|written|"
            r"(?:has|had) (?:read|written|written to|written into|stored)) "
    ...
    (
        ...
        re.compile(
            ...
            r"overrode|(?:has|had) overridden|"
            r"(?:is|are|was|were|has been|have been) (?:overriding|bypassing)|"
    ...
    (
        ...
        re.compile(
            ...
            r"(?:a )?verification[- ]bundles?\b"
            r"|\b(?:semantic[- ]cache |cache )?admission "
            r"(?:has|have|had) to (?:bypass|skip|omit|waive) "
            r"(?:a )?verification[- ]bundles?\b"
            r"|\b(?:semantic[- ]cache |cache )?admission "
            r"(?:need not|needn't) require (?:a )?verification[- ]bundles?\b"
            r"|\b(?:semantic[- ]cache |cache )?admission "
            ...
            r"(?:(?:(?:may|can|must|could|would|should|might) be|is|are) )?omitted "
            r"for (?:semantic[- ]cache|cache) admission\b"
            r"|\bverification[- ]bundle waivers? "
            r"(?:(?:is|are|was|were|has been|have been|"
            r"(?:may|can|must|could|would|should|might) be) )?"
            r"(?:allowed|permitted|approved|enabled|authorized|granted|available|supported) "
            r"for (?:semantic[- ]cache|cache) admission\b"
            r"|\bverification[- ]bundle requirements? "
            r"(?:(?:(?:may|can|must|could|would|should|might) be|is|are) )?"
            r"omitted for (?:semantic[- ]cache|cache) admission\b"
            r"|\bverification[- ]bundle requirements? "
            r"(?:(?:is|are|was|were|"
            r"(?:is|are|was|were|has been|have been) being|"
            r"(?:may|can|must|could|would|should|might) be) )?"
            r"(?:bypassed|waived|skipped|omitted) for (?:semantic[- ]cache|cache) admission\b"
            r"|\bverification[- ]bundle requirements? "
            r"(?:(?:has|have|had) to be )"
            r"(?:bypassed|waived|skipped|omitted) for (?:semantic[- ]cache|cache) admission\b"
            r"|\bverification[- ]bundle requirements? "
    ...
    (
        ...
        re.compile(
            ...
            r"(?:(?:is|are|was|were|has been|have been) |"
            r"(?:can|may|will|shall|must|could|would|should|might) be )"
            ...
            r"(?:(?:is|are|was|were|has been|have been) (?:now )?being |"
            r"(?:is|are|was|were|has been|have been) (?:now )?|"
            r"(?:is|are|was|were|has been|have been) still |"
            r"(?:still )?remains? |(?:has|have|had) remained |remained |"
            r"(?:became|becomes|has become|had become) |"
            r"(?:has|have|had) stayed |stays? |"
            r"(?:has|have|had) to be |"
            r"(?:can|may|will|shall|must|could|would|should|might) be )"
            r"(?:authorized|approved|allowed|enabled|permitted|granted) "
            ...
            r"has been serving|will serve|will be serving|must serve|must be serving) "
