...
- Current canonical OpenAPI command remains `make openapi`; decoupled backend/frontend targets are tracked as backlog work until implemented.
...
- Prefer `docker compose` v2 in new or edited commands; any `docker-compose` fallback is transitional and must be documented until migration is complete.
...
- Target-state extraction of AI runtime into a dedicated bounded context is tracked in `docs/roadmap/BACKLOG_LEDGER.md` until implemented.
