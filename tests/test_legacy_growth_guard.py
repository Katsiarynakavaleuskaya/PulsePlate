    ...
    return (
        "from app.bootstrap.application import app\n",
        {
            "app/bootstrap/application.py": textwrap.dedent("""
                ...
                """),
            "app/main.py": "from app.bootstrap.application import app\n",
...
def test_application_instance_ownership_accepts_bounded_lexical_fixture() -> None:
...
def test_application_instance_ownership_rejects_misplaced_direct_constructor(
    constructor: str,
) -> None:
    legacy_source, app_sources = _application_instance_ownership_sources()
    app_sources["app/other.py"] = f"rogue = {constructor}\n"

    errors = legacy_guard.validate_application_instance_ownership(legacy_source, app_sources)

    assert "direct FastAPI production constructor count must be exactly 1; found 2" in errors
    assert any(
        "app/other.py:1: direct FastAPI constructor is forbidden outside" in error
        for error in errors
    )
...
def test_application_instance_ownership_requires_exact_factory_body(
    old: str,
    new: str,
) -> None:
    legacy_source, app_sources = _application_instance_ownership_sources()
    ...
    assert any("factory body must be exactly" in error for error in errors)


def test_application_instance_ownership_keeps_alias_dataflow_outside_lexical_claim() -> None:
    legacy_source, app_sources = _application_instance_ownership_sources()
    app_sources["app/other.py"] = textwrap.dedent("""
        from fastapi import FastAPI as ImportedFastAPI

        def accepts_factory(factory: type[ImportedFastAPI]) -> type[ImportedFastAPI]:
            return factory
        """)
...
def test_application_instance_ownership_fails_closed_on_missing_or_invalid_source() -> None:
    legacy_source, app_sources = _application_instance_ownership_sources()
    del app_sources["app/bootstrap/application.py"]
    app_sources["app/other.py"] = "if True print('invalid')\n"

    errors = legacy_guard.validate_application_instance_ownership(legacy_source, app_sources)

    assert "app/bootstrap/application.py: required constructor owner is missing" in errors
