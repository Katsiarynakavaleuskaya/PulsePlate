...
def test_release_base_url_is_https() -> None:
    """Release BASE_URL must use HTTPS scheme."""
    with INFO_RELEASE_PLIST.open("rb") as fh:
        plist = plistlib.load(fh)
    base_url: str = plist.get("BASE_URL", "")
    assert base_url.startswith("https://"), f"Release BASE_URL must be HTTPS, got: {base_url}"


def test_release_base_url_is_canonical_host() -> None:
    """Release BASE_URL must point to the canonical host (operator decision)."""
    with INFO_RELEASE_PLIST.open("rb") as fh:
        plist = plistlib.load(fh)
    base_url: str = plist.get("BASE_URL", "")
    assert (
        "pulseplate.app" in base_url
    ), f"Release BASE_URL must use canonical host pulseplate.app, got: {base_url}"
