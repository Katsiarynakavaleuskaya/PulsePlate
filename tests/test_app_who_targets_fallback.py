...
from app.routers.legacy_premium_nutrition import api_who_targets
...
def test_api_who_targets_value_error_uses_loss_fallback(
    ...
    response = asyncio.run(api_who_targets(request.model_dump()))
...
def test_api_who_targets_unexpected_failure_is_not_false_200(
    ...
    with pytest.raises(HTTPException) as raised:
        asyncio.run(api_who_targets(_request().model_dump()))
...
def test_api_who_targets_returns_safety_warnings(
    ...
    response = asyncio.run(api_who_targets(_request().model_dump()))
