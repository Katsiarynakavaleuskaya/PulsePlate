...
def test_create_and_get_user(_cleanup_users: Any, client: TestClient) -> None:
...
def test_list_users_pagination(_cleanup_users: Any, client: TestClient) -> None:
    for idx in range(3):
        client.post(
            "/api/v1/users",
...
def test_create_user_conflict(_cleanup_users: Any, client: TestClient) -> None:
...
def test_get_user_not_found(_cleanup_users: Any, client: TestClient) -> None:
    response = client.get("/api/v1/users/9999")
    assert response.status_code == 404


def test_delete_user_success_and_not_found(_cleanup_users: Any, client: TestClient) -> None:
...
def test_create_user_validation_error(_cleanup_users: Any, client: TestClient) -> None:
