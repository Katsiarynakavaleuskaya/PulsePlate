...
def test_catalog_regions_stores_search_smoke(client: TestClient) -> None:
    ...
    assert regions.headers["content-type"].startswith("application/json")
    assert [row["id"] for row in regions.json()] == ["ES", "US"]

    stores = client.get("/api/v1/catalog/stores", params={"region_id": "ES"})
    assert stores.status_code == 200
    assert stores.headers["content-type"].startswith("application/json")
    ...
    assert search.headers["content-type"].startswith("application/json")
