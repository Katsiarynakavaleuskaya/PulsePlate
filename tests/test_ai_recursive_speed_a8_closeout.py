...
def _write_valid_repo(tmp_path: Path) -> None:
    ...
    _write(
        ...
        (
            "def retrieve_and_validate_rag() -> object:\n"
    ...
    _write(
        ...
        (
            "def _traced_retrieve_and_validate_rag(\n"
            "    recursive_optimization_hints: object | None = None,\n"
            ") -> object | None:\n"
            "    return recursive_optimization_hints\n"
        ),
    )
...
def test_checker_rejects_numeric_false_branch_early_stop_literals(tmp_path: Path) -> None:
    _write_valid_repo(tmp_path)
    path = tmp_path / "core/rag/recursive_retrieval.py"
    path.write_text(
        "def _make_optimization_stats() -> dict:\n"
...
def test_checker_rejects_non_iterable_for_loop_early_stop_literals(tmp_path: Path) -> None:
    _write_valid_repo(tmp_path)
    path = tmp_path / "core/rag/recursive_retrieval.py"
    path.write_text(
        "def _make_optimization_stats() -> dict:\n"
...
def test_checker_rejects_range_zero_loop_early_stop_literals(tmp_path: Path) -> None:
    _write_valid_repo(tmp_path)
    path = tmp_path / "core/rag/recursive_retrieval.py"
    path.write_text(
        "def _make_optimization_stats() -> dict:\n"
...
def test_checker_rejects_module_level_param_only_symbol_spoof(tmp_path: Path) -> None:
    _write_valid_repo(tmp_path)
    path = tmp_path / "core/rag/orchestration.py"
    path.write_text("recursive_optimization_hints = {}\n", encoding="utf-8")

    errors = _errors(tmp_path)

    assert any("core/rag/orchestration.py landed symbol" in error for error in errors)


def test_checker_rejects_alias_empty_loop_param_only_wiring(tmp_path: Path) -> None:
    _write_valid_repo(tmp_path)
    path = tmp_path / "app/services/insight_runtime.py"
    path.write_text(
        "EMPTY = ()\n"
...
def test_checker_rejects_range_zero_loop_param_only_symbol_wiring(tmp_path: Path) -> None:
    _write_valid_repo(tmp_path)
    path = tmp_path / "app/services/insight_runtime.py"
    path.write_text(
        "def _traced_retrieve_and_validate_rag() -> None:\n"
...
def test_checker_rejects_exposes_runtime_expansion_claim(tmp_path: Path) -> None:
    _write_valid_repo(tmp_path)
    roadmap = tmp_path / "docs/roadmap/PulsePlate_RAG_LLM_Karpathy_Epic_Pipeline.md"
    roadmap.write_text(
        _valid_roadmap().replace(
            "This closeout reconciles stale roadmap/backlog/review truth.",
            "PR-A8 exposes semantic cache in production.",
        ),
        encoding="utf-8",
    )
...
def test_checker_rejects_split_forbidden_surface_tail_without_repeated_subject(
    ...
    roadmap.write_text(
        _valid_roadmap().replace(
            "This closeout reconciles stale roadmap/backlog/review truth.",
            "PR-A8 semantic cache is not active, but enabled for live traffic.",
        ),
        encoding="utf-8",
    )
...
def test_checker_rejects_split_action_tail_with_inherited_surface_context(tmp_path: Path) -> None:
    _write_valid_repo(tmp_path)
    roadmap = tmp_path / "docs/roadmap/PulsePlate_RAG_LLM_Karpathy_Epic_Pipeline.md"
    roadmap.write_text(
        _valid_roadmap().replace(
            "This closeout reconciles stale roadmap/backlog/review truth.",
            "PR-A8 semantic cache remains closed, but used in production.",
        ),
        encoding="utf-8",
    )
...
def test_checker_allows_split_negated_activation_tail(tmp_path: Path) -> None:
    _write_valid_repo(tmp_path)
    roadmap = tmp_path / "docs/roadmap/PulsePlate_RAG_LLM_Karpathy_Epic_Pipeline.md"
    roadmap.write_text(
        _valid_roadmap().replace(
            "This closeout reconciles stale roadmap/backlog/review truth.",
            "PR-A8 semantic cache remains closed, but not active.",
        ),
        encoding="utf-8",
    )
...
@pytest.mark.parametrize(
    ...
    (
        ...
        "PR-A8 guarantees response time < 200ms.",
        "PR-A8 guarantees p95 under 200ms.",
    ),
)
def test_checker_rejects_non_percent_symbol_benchmark_overclaims(
    ...
    assert any("unvalidated benchmark claim" in error for error in errors), claim
...
@pytest.mark.parametrize(
    ...
    (
        (
            "PR-A8 does not open semantic cache therefore Redis production-ready rollout.",
            "forbidden PR-A8 runtime expansion claim",
        ),
        (
            "PR-A8 is not pending therefore active implementation lane.",
            "stale PR-A8 active/pending wording",
        ),
        (
            "PR-A8 does not regress quality therefore proves latency reduction 50-60% average.",
            "unvalidated benchmark claim",
        ),
        (
            "PR-A8 does not open semantic cache so Redis production-ready rollout.",
            "forbidden PR-A8 runtime expansion claim",
        ),
        (
...
def test_checker_rejects_therefore_split_bypasses(
    ...
    assert any(expected in error for error in errors), claim


def test_checker_rejects_hyphenated_pr_id_runtime_claim(tmp_path: Path) -> None:
    _write_valid_repo(tmp_path)
    roadmap = tmp_path / "docs/roadmap/PulsePlate_RAG_LLM_Karpathy_Epic_Pipeline.md"
    roadmap.write_text(
        _valid_roadmap().replace(
            "This closeout reconciles stale roadmap/backlog/review truth.",
            "PR-1506 enables semantic cache by default.",
        ),
        encoding="utf-8",
    )
