...
class TestCreateDailyPlate:
    ...
    def test_create_daily_plate_basic(
        self, mock_apply_boosters, mock_create_meal, mock_parse_recipe_db, mock_parse_food_db
    ):
    ...
    @patch("core.daily_plate.create_meal")
    @patch("core.daily_plate.apply_boosters_if_needed")
    def test_create_daily_plate_with_provided_databases(
        self, mock_apply_boosters, mock_create_meal, mock_parse_recipe_db, mock_parse_food_db
    ):
        """Test daily plate creation with provided databases."""
        # Mock meal creation
        mock_meal = {"name": "breakfast", "kcal": 500, "micro_coverage": {"iron_mg": 50}}
        mock_create_meal.return_value = mock_meal
...
class TestCreateMeal:
    ...
    def test_create_meal_with_recipe(
        self, mock_calc_micro, mock_calc_nutrients, mock_scale_recipe, mock_find_recipe
    ):
...
class TestFindRecipeForMeal:
    ...
    def test_find_recipe_for_breakfast(self):
        ...
        recipe_db = {"Овсянка с орехами": Mock(), "Гречка с тофу": Mock()}
    ...
    def test_find_recipe_for_lunch(self):
        ...
        recipe_db = {"Овсянка с орехами": Mock(), "Гречка с тофу": Mock(), "Рис с курицей": Mock()}
        recipe_db["Овсянка с орехами"].flags = {"VEG"}
        recipe_db["Гречка с тофу"].flags = {"VEG"}
