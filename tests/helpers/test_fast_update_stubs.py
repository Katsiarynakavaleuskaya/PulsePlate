...
def test_fast_update_stubs_smoke() -> None:
    ...
    result_default = asyncio.run(scheduler.force_update())
    result_with_source = asyncio.run(scheduler.force_update("usda"))
    assert "usda" in result_default
    assert "usda" in result_with_source
    assert result_default["usda"].success is True
    assert result_with_source["usda"].success is True
...
def test_make_scheduler_stub_allows_falsy_payload() -> None:
    ...
    result = asyncio.run(scheduler.force_update("usda"))
    assert result["usda"] == {}


def test_patch_unified_db_common_foods_fast_returns_unified_items(
...
def test_iter_background_modules_skips_missing_legacy_alias(
    ...
    assert "legacy_app" in attempted_imports
