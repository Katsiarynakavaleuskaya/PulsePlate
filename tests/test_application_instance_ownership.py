...
def _run_import_scenario(imports: str) -> list[object]:
    ...
    result = completed.stdout.rsplit("OWNERSHIP_RESULT=", 1)[-1].splitlines()[0]
...
def test_bespoke_http_owner_states_fail_closed(path: str, method: str, owners: str) -> None:
    target = FastAPI()
    fn = route_endpoint_for_path_method(main.app.routes, path, method)
    for owner in owners:
        if owner == "i":
...
def test_source_guard(source: str, state: str, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    routes = [Route(path, endpoint, methods=["POST"])] if state == "raw" else []
    selected = APIRouter(routes=routes)
    if state == "extra":
    ...
    target = FastAPI()
    before = (tuple(target.routes), tuple(target.user_middleware))
    with pytest.raises(RuntimeError, match="Invalid canonical HTTP source route"):
        main.ensure_canonical_app_bootstrap(target)
    assert (tuple(target.routes), tuple(target.user_middleware)) == before
