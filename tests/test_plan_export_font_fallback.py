...
def test_register_font_fallback_when_font_missing(
    ...
    def fake_exists(self: Path) -> bool:
        """Mock Path.exists to return True only for the fake_path."""
...
def test_register_font_fallback_when_font_path_missing(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Test font fallback when FONT_PATH does not exist (exists() returns False)."""
    fake_path = Path("/__no_such__/DejaVuSans.ttf")
    monkeypatch.setattr(pe, "FONT_PATH", fake_path)
