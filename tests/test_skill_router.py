...
EXPECTED_PRIVILEGED_SURFACE_POLICY_LINES: tuple[str, ...] = (
    ...
    "- dependency, quality-gate, coverage-governance, toolchain, local-hook, edge-deploy, and repo-governance control files matching `AGENTS.md`, `RUNBOOK_AGENT.md`, `Makefile`, `.bandit`, `.bandit.yaml`, `.coveragerc`, `.coderabbit.yaml`, `.gitmodules`, `.nvmrc`, `.python-version`, `.ruby-version`, `.secrets.baseline`, `.sourcery.yaml`, `.tool-versions`, `.trivyignore`, `.yamllint`, `.pre-commit-config.yaml`, `.pre-commit-config.yml`, `codecov.yml`, `codecov.yaml`, `.github/CODEOWNERS`, `.github/PULL_REQUEST_TEMPLATE/*.md`, `.github/actionlint.yml`, `.github/actionlint.yaml`, `.github/dependabot.yml`, `.github/dependabot.yaml`, `.github/pull_request_template.md`, `frontend/wrangler.toml`, `mcp-config.json`, `opencode.json`, `pyproject.toml`, root/frontend `package*.json`, `ios/Gemfile*`, `ios/Package.swift`, `ios/Package.resolved`, Xcode project and SwiftPM manifests, `ios/PulsePlate/PrivacyInfo.xcprivacy`, `requirements*.txt`, `requirements*.in`, `constraints*.txt`, `scripts/deploy.sh`, `scripts/diagnose_web.sh`, `scripts/hooks/repo_python.sh`, `scripts/ops/postgres_backup.sh`, `scripts/ops/postgres_restore.sh`, `scripts/redeploy_caddy.sh`, `scripts/run-backend-tests-pre-commit.sh`, `scripts/validate-ci-environment.sh`, `scripts/ci_*.sh`, `scripts/deploy_*.sh`, `tests/test_repo_policy_guards.py`, `worker.js`, `wrangler.toml`, policy guard tests under `tests/guards/**`, and repo-local hook scripts under `.githooks/**`",
