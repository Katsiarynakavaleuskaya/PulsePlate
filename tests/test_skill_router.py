...
EXPECTED_PRIVILEGED_SURFACE_POLICY_LINES: tuple[str, ...] = (
    ...
    "- container, deploy, devcontainer, security-lab, backup-systemd, and security-scan policy under `Dockerfile`, `.dockerignore`, `.trivyignore`, `.devcontainer` Docker/Compose/devcontainer files, deploy Caddy/Compose patterns, METATRON lab compose/guard files, backup systemd unit examples, `frontend/.dockerignore`, `frontend/Dockerfile.caddy-spa`, and `trivy/**`",
    "- dependency, quality-gate, coverage-governance, toolchain, local-hook, skill-source, Cursor control-plane, edge-deploy, iOS release-control, AgentGuard scanner, and repo-governance control files matching root or scoped `AGENTS.md`, `RUNBOOK_AGENT.md`, `Makefile`, `.bandit`, `.bandit.yaml`, `.coveragerc`, `.coderabbit.yaml`, `.cursor/mcp.json.example`, `.gitmodules`, `.nvmrc`, `.python-version`, `.ruby-version`, `.secrets.baseline`, `.sourcery.yaml`, `.tool-versions`, `.trivyignore`, `.yamllint`, `.pre-commit-config.yaml`, `.pre-commit-config.yml`, `codecov.yml`, `codecov.yaml`, `.github/CODEOWNERS`, `.github/PULL_REQUEST_TEMPLATE/*.md`, `.github/actionlint.yml`, `.github/actionlint.yaml`, `.github/dependabot.yml`, `.github/dependabot.yaml`, `.github/pull_request_template.md`, `frontend/wrangler.toml`, `mcp-config.json`, `opencode.json`, `pyproject.toml`, root/frontend `package*.json`, `ios/Gemfile*`, `ios/Package.swift`, `ios/Package.resolved`, Xcode project and SwiftPM manifests, `ios/PulsePlate/Info-Release.plist`, `ios/PulsePlate/PulsePlate.entitlements`, `ios/PulsePlate/PrivacyInfo.xcprivacy`, `ios/PulsePlate/*/InfoPlist.strings`, `requirements*.txt`, `requirements*.in`, `constraints*.txt`, `scripts/deploy.sh`, `scripts/diagnose_web.sh`, `scripts/hooks/repo_python.sh`, `scripts/install_codex_skills.sh`, `scripts/ops/postgres_backup.sh`, `scripts/ops/postgres_restore.sh`, `scripts/redeploy_caddy.sh`, `scripts/run-backend-tests-pre-commit.sh`, `scripts/validate-ci-environment.sh`, `scripts/ci_*.sh`, `scripts/deploy_*.sh`, `tests/security/_api_authz_contracts.py`, `tests/security/test_api_authz_contract_static.py`, `tests/test_repo_policy_guards.py`, `worker.js`, `wrangler.toml`, policy guard tests under `tests/guards/**`, repo-local hook scripts under `.githooks/**`, GitHub helper scripts under `.github/scripts/**`, skill sources under `.agents/skills/**`, `tools/agentguard/**`, `tools/codex_skills/**`, `tools/cybersecurity_skills/**`, and Cursor control-plane files under `.cursor/commands/**` and `.cursor/rules/**`",
