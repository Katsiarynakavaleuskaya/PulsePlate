...
class TestPremiumTargetsComprehensive:
    ...
    def test_premium_targets_required_micronutrients_male_19_50(self):
        ...
        # Check that all required micronutrients are present with correct values from COMPACT_MICRONUTRIENT_RDA
        expected_values = {
        ...
        for nutrient, expected_value in expected_values.items():
            ...
            assert priority_micros[nutrient] == expected_value, f"Nutrient {nutrient} has incorrect value: expected {expected_value}, got {priority_micros[nutrient]}"
    ...
    def test_premium_targets_macronutrient_distribution(self):
        ...
        assert expected_fat_min <= macros["fat_g"] <= expected_fat_max, \
            f"Fat target {macros['fat_g']}g not in expected range {expected_fat_min}-{expected_fat_max}g"
    ...
    def test_premium_targets_hydration_target(self):
        ...
        assert (expected_water - tolerance) <= result["water_ml"] <= (expected_water + tolerance), \
            f"Water target {result['water_ml']}ml not in expected range {expected_water-tolerance}-{expected_water+tolerance}ml"
    ...
    def test_premium_targets_kcal_safety_bounds(self):
        ...
        result = response.json()
        assert result["kcal_daily"] >= 1200, f"Calorie target {result['kcal_daily']} below minimum safe level of 1200 kcal"
    ...
    def test_premium_targets_tdee_vs_bmr(self):
        ...
        # We can't directly compare to BMR without internal calculations, but we can check it's reasonable
        assert result["kcal_daily"] > 1500, f"TDEE {result['kcal_daily']} seems too low for a 70kg male"
