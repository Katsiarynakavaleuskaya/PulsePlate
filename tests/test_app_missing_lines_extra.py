...
class TestAppMissingLinesExtra:
    ...
    def test_lifespan_error_branches(self):
        ...
        with patch.object(app_mod, "start_background_updates", _boom_start), \
    ...
    def test_bmi_pregnancy_visualization_branch(self):
        ...
        with patch.object(app_mod, "generate_bmi_visualization", lambda **kw: {"available": True, "x": 1}):
            payload = {
                "weight_kg": 60.0,
                "height_m": 1.65,
                "age": 29,
    ...
    def test_insight_implicit_disabled_flag_branch(self):
        ...
        with patch.dict(os.environ, {"FEATURE_INSIGHT": "maybe"}, clear=False), \
    ...
    def test_premium_bmr_value_and_http_errors(self):
        ...
        with patch.object(app_mod, "calculate_all_bmr", side_effect=ValueError("bad")), \
            ...
            r = self.client.post("/api/v1/premium/bmr", json=data, headers={"X-API-Key": "test_key"})
            assert r.status_code == 400

        # Trigger HTTPException passthrough re-raise (818)
        with patch.object(app_mod, "calculate_all_bmr", side_effect=HTTPException(status_code=418, detail="teapot")), \
             patch.object(app_mod, "calculate_all_tdee", lambda *a, **k: {}), \
             patch.object(app_mod, "get_activity_descriptions", lambda: {}):
            data = {
                "weight_kg": 70.0,
    ...
    def test_premium_plate_missing_bmr_tdee_check(self):
        ...
        with patch.object(app_mod, "calculate_all_bmr", None), \
            ...
            r = self.client.post("/api/v1/premium/plate", json=payload, headers={"X-API-Key": "test_key"})
            assert r.status_code == 503

    def test_nutrient_gaps_value_error(self):
        # Hit 1275 by raising ValueError from build_nutrition_targets
        with patch.object(app_mod, "analyze_nutrient_gaps", lambda *a, **k: {}), \
            ...
            r = self.client.post("/api/v1/premium/gaps", json=payload, headers={"X-API-Key": "test_key"})
            assert r.status_code == 400
    ...
    def test_export_pdf_generic_errors(self):
        ...
        with patch.object(app_mod, "to_pdf_day", side_effect=RuntimeError("x")):
            r = self.client.get("/api/v1/premium/exports/day/plan123.pdf", headers={"X-API-Key": "test_key"})
            assert r.status_code == 500

        with patch.object(app_mod, "to_pdf_week", side_effect=RuntimeError("y")):
            r = self.client.get("/api/v1/premium/exports/week/plan123.pdf", headers={"X-API-Key": "test_key"})
            assert r.status_code == 500
