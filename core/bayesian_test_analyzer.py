...
class BayesianTestAnalyzer:
    ...
    def __init__(self, data_file: Optional[Path] = None) -> None:
        ...
        # Управление персистом истории через переменные окружения
        ...
        self.data_file = Path(history_path) if history_path else Path("test_execution_history.json")
    ...
    def _calculate_evidence(
        self, symptoms: Set[str], similar_cases: List[TestExecution]
    ) -> float:
