...
def _calculate_macro_targets(target_kcal: int, profile: UserProfile) -> MacroTargets:
    ...
    # Ensure a small positive floor for carbohydrates to avoid zero-carb edge cases
    # observed under aggressive deficit + high protein/fat multipliers.
    carbs_g = max(1, int(round(carbs_kcal / 4)))
