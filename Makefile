...
bandit-full:
	...
	@if [ "$(CI)" = "true" ]; then \
		...
		bandit -r app core scripts providers tests alembic $(wildcard *.py) -c .bandit --severity-level medium -f json -o bandit-report.json; \
	else \
		echo "$(YELLOW)Local mode: разрешающий режим (warnings only)...$(NC)"; \
		bandit -r app core scripts providers tests alembic $(wildcard *.py) -c .bandit --severity-level medium -f json -o bandit-report.json || true; \
	fi
