...
- `user_knowledge` разрешён только при authenticated `subject_id`; если subject context отсутствует, vector retrieval обязан fail-closed и перейти на non-personal fallback. **Evidence:** `core/rag/vector_rag.py:201` (fail-closed on missing `subject_id`), `core/rag/vector_rag.py:317` (fallback to Jaccard on empty/error vector path), `core/rag/orchestration.py:137` and `core/rag/orchestration.py:147` (subject propagation into recursive/vector retrieval), `app/routers/legacy_insight.py:49` and `app/services/insight_compat.py:97` (authenticated `/api/v1/insight` derives and forwards `subject_id`), `app/services/insight_compat.py:116` (legacy `/insight` omits subject context), `app/routers/cbt_insight.py:176` (PRO CBT endpoint derives and forwards `subject_id`).
