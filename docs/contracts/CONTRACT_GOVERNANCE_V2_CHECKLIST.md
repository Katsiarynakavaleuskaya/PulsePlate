...
1. **Backend to Web:** Automatic sync via OpenAPI generation (`make openapi`)
2. **Backend to iOS:** Manual sync; breaking changes must include iOS DTO update PR (or ledger item)
3. **Web to Backend:** Frontend must not assume schema; always use generated types
4. **iOS to Backend:** Mobile must handle unknown fields gracefully (future-proofing)
