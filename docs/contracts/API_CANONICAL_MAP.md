...
| FitChef support handoff | `/api/v1/pro/fitchef/recommend` | POST | PRO | Landed via PR #2320 / `f95a329d899d5ac4fa73f198e90cfed44d0fc45c` as a feature-gated deterministic descriptor-only route with no navigation, execution, or plan-mutation authority; utility remains unmeasured |
| FitChef support outcome | `/api/v1/pro/fitchef/recommend/outcome` | POST | PRO | Default-off, rate-limited, credential-subject-scoped intake for `acknowledged`/`dismissed` client assertions; it proves no click, prior handoff, consent, navigation, plan execution, effectiveness, or causality |
...
- `POST /api/v1/pro/fitchef/recommend` landed via PR #2320 on 2026-08-25 at `f95a329d899d5ac4fa73f198e90cfed44d0fc45c`. Its `target_surface` is an opaque backend-owned product-surface slug, not proof of client navigation, downstream execution, plan mutation, deployment, or product value. The route accepts only an exact case-insensitive `application/json` base media type before the first `;`; every rejected media type uses the stable JSON `422` envelope.
- `POST /api/v1/pro/fitchef/recommend/outcome` is a separate default-off intake with canonical PRO auth, a 4096-byte streamed-body cap, strict closed JSON, race-safe per-subject idempotency, forced PostgreSQL RLS, support-led export/delete, and a 12-series metric ceiling. It does not retroactively strengthen the `/recommend` descriptor or the PR #2337 frontend evidence.
