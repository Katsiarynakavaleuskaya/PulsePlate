...
2. **`build.yml` — publish to GHCR (scan-before-push)** — **`platforms: linux/amd64`** only (reduces cache/export surface; restore `linux/arm64` when stable), same scoped cache pattern as (1), **`provenance: false`** on the load-only scan build, Trivy fail-closed before registry login/push, and GitHub-signed provenance/SBOM attestations after the same scanned tags are pushed and the registry digest is known.
...
5. **Historical smoke build lane** — removed during build-path consolidation; the remaining PR-time smoke checks run from `build.yml` load-only validation.
...
- Keep **`provenance: false`** on `load: true` paths listed in those anchors. For
  `build.yml` publish, scan the loaded production image before GHCR push and
  then create GitHub-signed provenance/SBOM attestations for the pushed digest.
  For CD pushed-image BuildKit lanes, use **`provenance: mode=min`** plus
  **`sbom: true`** while private package-index inputs flow through BuildKit
  secret envs.
...
- Trade-off remains scoped: pushed-image GitHub attestations are restored, but
  `load: true` paths keep temporary constraints and `build.yml` publish avoids a
  rebuild-after-scan gap.
...
## Consequences
- Image consumers can verify GitHub-signed provenance for GHCR images produced by
  pushed-image lanes in `build.yml` and `cd.yml`.
...
- `.github/workflows/build.yml:396-535` publish job: `linux/amd64`,
  `push: false`, `load: true`, scoped `cache-from` / `cache-to`,
  `provenance: false`, Trivy fail-closed before GHCR login/push, then push the
  scanned tags, attest the pushed digest, and verify attestations before release
  control-plane digest publication.
...
- `.github/workflows/cd.yml:478-545` production image: scoped
  `cache-from` / `cache-to`, `provenance: mode=min`, `sbom: true`,
  exact-digest attestation verification before deploy.
...
- [x] Restore pushed-image provenance for `build.yml` publish through the
  scan-before-push lane: the loaded scan build keeps `provenance: false`, then
  the scanned tags are pushed and GitHub-signed provenance/SBOM attestations are
  created against the pushed digest (PR #1448, refined by PR #1935).
