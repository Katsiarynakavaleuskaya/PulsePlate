...
- [ ] P1: Unimplemented feature keys backlog (SoT = tests/feature_manifest.py)
  ...
  - Priority: P1
  - Target PR: PR-748
  - Status: 🔄 In progress (PR-891: legacy_bmi_removed enabled; 15 total enabled, 5 remaining)
  - Area: backend / tests / feature debt management
  ...
  - Implemented keys (latest):
    ...
    - `legacy_bmi_removed` -> ✅ Enabled (PR-891, 2026-02-24); implemented canonical `estimate_level()` in `core/bmi/engine.py` (PRO tier fitness experience level); added `FitnessLevel` type alias, `FITNESS_LEVEL_DISPLAY_NAMES` (ru/en/es), `get_fitness_level_display()` helper; updated `bmi_core.py` shim to delegate; implemented tests in `test_app_coverage_unit_combined.py`, `test_level_es.py`, `test_bmi_core_shim_diffcover.py`; key removed from FEATURE_TODO_KEYS
  - Keys still gated (module exists but tested API surface incomplete):
    - `planner_engines`, `plate_day_micros`, `exports_recipes_products`, `sports_disclaimers_lifestage`, `nutrient_recommendations`
