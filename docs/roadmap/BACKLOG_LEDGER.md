...
- [ ] P1: TestClient lifecycle and session-fixture isolation cleanup
  - Owner: @katsiaryna_kavaleuskaya
  - Priority: P1
  - Target PR: PR #2312 (TC2-09), merged PR #2325 / PR #2330 stabilization, merged PR #2332 direct-getter ownership stabilization, merged PR #2334 (TC2-09B), merged PR #2342 (TC2-09C), merged [PR #2351](https://github.com/Katsiarynakavaleuskaya/PulsePlate/pull/2351) (TC2-09D, squash merge `e58f911c372bb46e1f0e99436feb0ca34c22b82d`), merged [PR #2353](https://github.com/Katsiarynakavaleuskaya/PulsePlate/pull/2353) (TC2-09E, squash merge `74b3ef863d3f663400c11a11e0f9aa37012b2fdf`), merged [PR #2357](https://github.com/Katsiarynakavaleuskaya/PulsePlate/pull/2357) (TC2-09F, squash merge `cf096f335a53c1ce056f570142ca9b20a13eb0b1`), and current [PR #2361](https://github.com/Katsiarynakavaleuskaya/PulsePlate/pull/2361) (TC2-09G, `codex/testclient-tc2-09g-bayes-adherence-lifecycle`); terminal TC2-10 remains TBD
  - Status: 🟡 In progress — TC2-09F / PR #2357 is merged; TC2-09G / PR #2361 is the current bounded one-site Bayesian adherence isolated-client carrier from synchronized exact base `a157d445c98c3e4bea76bd95c2a8d333c99725c1`; the tracker remains open and TC2-10 remains blocked
  ...
  - Progress:
    ...
    - Current TC2-09G Bayesian adherence lifecycle carrier: [PR #2361](https://github.com/Katsiarynakavaleuskaya/PulsePlate/pull/2361) on branch `codex/testclient-tc2-09g-bayes-adherence-lifecycle`, against synchronized exact base `a157d445c98c3e4bea76bd95c2a8d333c99725c1`, migrates the sole recognized direct construction in `tests/test_bayes_adherence_api.py` to the existing function-scoped `isolated_test_client` with explicit `pro_headers` or `vip_headers` on every request, without changing shared fixtures or runtime. The same `10 → 10` collected nodes retain `17 → 17` HTTP source sites and `22 → 22` runtime requests with exact outcomes `18 × 200` and `4 × 422`; all nine JSON parses require JSON Content-Type first. Exact default, meal, slip, `n=6`, `n=7`, custom-key, and PRO/VIP state assertions prevent stale or cross-principal analyzer state from false-greening. The remeasured bounded AST census remains 363 sites / 89 files total and 362 / 88 outside `tests/_client.py` at that base, moving to 362 / 88 total and 361 / 87 outside the helper, with one canonical helper construction and target recognized sites `1 → 0`. The recognized target construction and the class-owned manual client/DB cleanup lifecycle owner each move from `1 → 0`; the finite literal guard gains only the exact removed target spelling.
...
- [ ] P1: ER-IOS-4 single Home FitChef Coach entry compatibility alias
  ...
  - Reason (EN): Historical ER-IOS-4 naming identifies the one discoverable
    ...
    carrier. It must not create capability-specific Home actions or duplicate
    the Home carrier's state, copy, navigation, or evidence ownership.
  - Links:
    - `ios/PulsePlate/Views/HomeView.swift`
    - `docs/design/PULSEPLATE_BUTTON_ACTION_PROMPT_MATRIX.md`
    - `docs/contracts/FITCHEF_STRUCTURED_COACH_CONTRACT.md`
  ...
  - DoD:
    - the canonical paid-ready Home action set contains exactly one localized
      FitChef Coach entry targeting the merged Hub destination
    - no tab, separate AI/recommendation/outcome action, raw-key entitlement
      inference, analytics, or second Home implementation carrier is added
    - deterministic single-entry, gate, localization, minimum-target,
      VoiceOver, lazy-construction, and route tests close in the canonical PR
...
- [ ] P1: iOS Consumer-first Home planning experience
  ...
  - Reason (EN): The current iOS Home is an engineering dashboard with raw-key
    ...
    backend-owned subscription flow plus local readiness for backend validation
    into five consumer states, exposes one `FitChef Coach` entry for paid-ready
    users, and routes to existing destinations without moving entitlement,
    profile, plan, nutrition, or billing truth into SwiftUI.
  - Exact IN:
    - `HomeExperienceState` with `loading`, `freeReady`, `paidNeedsProfile`,
      `paidReady`, and `unavailable`, derived only from the existing
      `SubscriptionManager` flow/snapshot and local `ProfileProviding`
      readiness; `SubscriptionManager` remains the sole freshness owner
    - Consumer Home primary actions for BMI, profile completion, today's plate,
      or retry; paid-ready always exposes one `FitChef Coach`, while Week and
      Shopping List remain controlled together by the existing
      `weeklyPlanReaderEnabled` flag
    ...
    - Inside that one Coach entry, `planningDirection` is always present and
      `aiGuidance` is added only when the existing `aiInsightEnabled` flag is on
    ...
    - EN/RU/ES copy remediation, state/navigation tests, repo-local render and
      accessibility evidence, same-PR ledger update, canonical mapping, and
  ...
  - Exact OUT:
    ...
    - Open Design, Figma, Drive, Canva, Web Creative, or any external design
      project, package, snapshot, prompt, token, or asset write
  - Links:
    ...
    - `docs/roadmap/BACKLOG_LEDGER.md#ledger-p1-ios-v3-pro-tools-rollout-alignment`
  - DoD:
    - All five states are deterministic and fail closed; raw key presence,
      StoreKit transaction, local tier string, refresh failure, unsupported
      snapshot status, or any inconsistent flow/snapshot pair never opens paid
      UI. This carrier does not classify snapshot age; manager-owned flow and
      snapshot consistency are the freshness boundary.
    ...
    - Paid-ready contains exactly one `FitChef Coach`; Week and Shopping List
      appear only under the existing controlled planning flag, and no separate
      AI/support action or technical copy remains on Home
    ...
    - Evidence remains repo-local and gitignored where generated; no external
      design project is opened or changed by this carrier
  - Rollback:
    - Revert the whole Home implementation PR to the previous Home router. No
      backend, DB, billing, entitlement, plan, data, migration, asset, or
      external-design repair is required.
...
- [x] P1: Remediate Ruby `json` CVE-2026-54696 in iOS release tooling
  ...
  - Links:
    - `docs/security/CVE-2026-54696-json-fastlane.md`
...
- [ ] P1: Centralize the current mapped ORM registration action
  ...
  - Target PR: [PR #2349](https://github.com/Katsiarynakavaleuskaya/PulsePlate/pull/2349) (`codex/canonical-orm-model-registration`)
...
- [ ] P1: Governed creative-code execution lane (PR-0 through PR-6)
  ...
  - Shadow Bayesian lifecycle forecast/scoring v1 and one prospective Pilot:
    ...
    - DoD: merged PR `#2299` added the closed three-family fixed-prior contract, exact validated baseline/outcome lineage, stable fixed-root forecast/start/score slots, strict no-replace publication, and paired optional `generate-candidate` binding without changing legacy PR-2 schemas or authority. ER-P5 remains byte-preserved as historical evidence of a pre-generation governance stop, not a product or Runner outcome. A direct external operator decision separately authorized exact ER-P6 in PR `#2308` as a new one-off RAG Pilot rather than a retry, relabel, overwrite, or substitute inside ER-P5; no further Pilot or target is inferred after ER-P6. Publish one immutable ER-P6 score at canonical terminal stop or the fixed 14-day cutoff, including honest `valid_but_unscored` or `measurement_invalid` state where applicable. Any `5000 bps` / `250000 ppm` result demonstrates measurement feasibility only; calibration, reliability, predictive skill, causal effectiveness, and product value remain `not_assessed`.
  ...
  - Resolved carryover remediation:
    - Owner: @katsiaryna_kavaleuskaya (Orchestration / Security)
    - Priority: P1
    - Target PR: merged PR `#2112` (`codex/adaptive-context-pack-lineage-fix`; follow-up to PR `#2101`)
    - Status: ✅ Completed by merge `95325b12f5a664a131ce79543f6a963c06f4de0a` on July 13, 2026; ER-P6 exact resume passed without widening the retained source contract.
