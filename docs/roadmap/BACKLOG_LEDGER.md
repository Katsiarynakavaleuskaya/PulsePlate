...
- [ ] P1: TestClient lifecycle and session-fixture isolation cleanup
  - Owner: @katsiaryna_kavaleuskaya
  - Priority: P1
  - Target PR: PR #2312 (TC2-09), merged PR #2325 / PR #2330 stabilization, merged PR #2332 direct-getter ownership stabilization, merged PR #2334 (TC2-09B), merged PR #2342 (TC2-09C), and current TC2-09D branch `codex/testclient-tc2-09d-health-readiness` (PR pending until open); terminal TC2-10 remains TBD
  ...
  - Progress:
    ...
    - Current TC2-09D health/readiness lifecycle carrier: branch `codex/testclient-tc2-09d-health-readiness` (PR pending until open) is synchronized to exact base `1b0b5fc9c8b06f8bee3c2b158b82581a333aeccb` and migrates exactly ten recognized direct construction sites across two test files: eight in `tests/test_health_db.py` (six `app.app`, two `legacy_app.app`) and the two `legacy_app.app` contexts exposed by the same bounded guard expansion in `tests/test_canonical_application_lifespan.py`. The measured working-tree post-change census using the same bounded AST recognizer is 368 sites / 91 files total and 367 / 90 outside `tests/_client.py`, with one canonical helper construction, zero recognized sites in each selected file, and zero across the combined ten-site cohort. The literal guard and census do not claim arbitrary aliases, formatting variants, wrappers, reflection, dynamic lookup, generated code, or runtime mutation.
    - Residual boundary: the synchronized exact TC2-09D base `origin/main@1b0b5fc9c8b06f8bee3c2b158b82581a333aeccb` contained 378 sites / 93 files total and 377 / 92 outside `tests/_client.py`; the current working diff reduces that bounded recognizer census to a measured 368 / 91 total and 367 / 90 outside the helper. The lifecycle checkbox and umbrella tracker remain open. TC2-10 remains blocked until recognized outside-helper construction callers reach zero and every terminal tracker condition passes.
