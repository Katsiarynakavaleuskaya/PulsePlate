...
- [ ] P1: TestClient lifecycle and session-fixture isolation cleanup
  - Owner: @katsiaryna_kavaleuskaya
  - Priority: P1
  - Target PR: PR #2312 (TC2-09), merged PR #2325 / PR #2330 stabilization, merged PR #2332 direct-getter ownership stabilization, merged PR #2334 (TC2-09B), merged PR #2342 (TC2-09C), and current [PR #2351](https://github.com/Katsiarynakavaleuskaya/PulsePlate/pull/2351) (`codex/testclient-tc2-09d-health-readiness`); terminal TC2-10 remains TBD
  - Status: 🟡 In progress — TC2-09C / PR #2342 merged as `57a80c130f4ae7317b9b905b02fa116551a88076`; TC2-09D is the current bounded ten-site, two-test-file carrier; the tracker remains open and TC2-10 remains blocked
  ...
  - Progress:
    ...
    - Current TC2-09D health/readiness lifecycle carrier: [PR #2351](https://github.com/Katsiarynakavaleuskaya/PulsePlate/pull/2351) on branch `codex/testclient-tc2-09d-health-readiness` is synchronized to exact base `59008784ba9e90dfe5a2fef87d1cdd9f65ce4b78` and migrates exactly ten recognized direct construction sites across two test files: eight in `tests/test_health_db.py` (six `app.app`, two `legacy_app.app`) and the two `legacy_app.app` contexts exposed by the same bounded guard expansion in `tests/test_canonical_application_lifespan.py`. The measured current-PR post-change census using the same bounded AST recognizer is 367 sites / 91 files total and 366 / 90 outside `tests/_client.py`, with one canonical helper construction, zero recognized sites in each selected file, and zero across the combined ten-site cohort. The literal guard and census do not claim arbitrary aliases, formatting variants, wrappers, reflection, dynamic lookup, generated code, or runtime mutation.
    - Residual boundary: the synchronized exact TC2-09D base `origin/main@59008784ba9e90dfe5a2fef87d1cdd9f65ce4b78` contained 377 sites / 93 files total and 376 / 92 outside `tests/_client.py`; the current PR material reduces that bounded recognizer census to a measured 367 / 91 total and 366 / 90 outside the helper. The lifecycle checkbox and umbrella tracker remain open. TC2-10 remains blocked until recognized outside-helper construction callers reach zero and every terminal tracker condition passes.
