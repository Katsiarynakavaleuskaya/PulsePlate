...
- [ ] P1: TestClient lifecycle and session-fixture isolation cleanup
  - Owner: @katsiaryna_kavaleuskaya
  - Priority: P1
  - Target PR: PR #2312 (TC2-09), PR #2325 and PR #2330 post-merge stabilization; terminal TC2-10 remains TBD
  - Status: 🟡 In progress — TC2-09 merged; PR #2330 main stabilization active; tracker remains open
  ...
  - Progress:
    ...
    - Main stabilization: PR #2325 merged the first bounded post-TC2-09 fixture/lifecycle repair; PR #2330 repairs the complete remaining active non-slow strict-request cohort of 16 nodes / 17 requests across 11 VIP coverage files after exact-main shard cancellation hid six positive callers from the initial JUnit inventory. The PR also removes three incidental raw constructors in already-touched nodes: its exact base census is 387 sites / 96 files (386 / 95 outside `tests/_client.py`) and its material head census is 384 / 96 (383 / 95 outside the helper). The checkbox and tracker remain open, `TC2-09B REQUIRED`, and TC2-10 remains blocked.
...
- [ ] P1: FitChef deterministic PRO support handoff
  ...
  - DoD:
    ...
    - Route-specific rollback is `git revert` of the exact E1-05 carrier; disabling the shared structured-coach feature flag is a family-wide emergency rollback that also disables PRO `/explain` and VIP `/insight`, not a bounded E1-05-only rollback. No data migration or state repair is required
