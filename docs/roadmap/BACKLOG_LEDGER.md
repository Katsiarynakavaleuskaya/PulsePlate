...
- [ ] P1: Split backend OpenAPI generation from frontend type generation
  ...
  - Area: build / contracts / developer workflow
  - Finding Type: workflow hardening
  - Reason: `make openapi` currently depends on `frontend-install`, so backend schema generation is coupled to Node/npm availability. This increases failure surface for backend-only work and conflicts with the desired contract-first workflow.
  - Links:
    - `Makefile`
    - `AGENTS.md`
    - `docs/runbooks/ENGINEER_QUICKPATH.md`
  ...
  - DoD:
    ...
    - Dedicated frontend type-generation target exists
    - `make openapi-check` remains the canonical sync verifier
    - `AGENTS.md`, runbooks, and CI docs reflect the split workflow without ambiguity
