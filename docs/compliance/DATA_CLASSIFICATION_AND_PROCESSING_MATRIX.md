...
| `/insight`, `/api/v1/insight`, `/api/v1/pro/cbt/insight` | Runtime + minimized audit/tracing metadata | AI-generated wellness analysis | Derived sensitive | Provider/deployment specific; local audit/tracing metadata is minimized or fingerprint-only | Local direct-user artifacts via DSAR map; provider-side artifacts follow provider terms | Yes, when a configured provider family is enabled |
