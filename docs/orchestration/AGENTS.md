...
- Teleology's user-impact rubric and start/change/QA/completion procedure live only in [`workflow.md#goal-to-outcome-review`](./workflow.md#goal-to-outcome-review). The validated `full`/`compact` prompt delivers instructions; Task Analysis, Work Review and DoD record execution. Neither the prompt nor a review label supplies human acceptance, legal certification or merge authority.
