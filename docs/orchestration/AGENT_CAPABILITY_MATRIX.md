...
- `prompt-engineering-eval-agent` designs prompt/eval protocols and SC-G3 false-hit observability; it does not replace `data-scientist-agent` for broader experiment design and does not own Sora visual prompt assets.
- `project-planning-agent` drafts plans, risk registers, and backlog mappings; it does not replace `agent-coordinator` routing, PR disposition, or merge-readiness authority.
