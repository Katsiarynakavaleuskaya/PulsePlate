...
- `creative_protocol_context_map.v1.schema.json` — immutable historical replay
  contract for already-emitted per-diff evidence; never generated by the
  current builder.
- `creative_protocol_context_map.v3.schema.json` — current final-material-only
  context-map contract. Version `v2` remains owned by the existing
  `creative-production-adjacent-pilot-v2` contract and is not repurposed.
...
## Codex Security Final-Material Budget Guard
...
-> fixes + local/current-head gates + material freeze
-> exact-head pulseplate-pr-review
-> one operator-issued final manual Codex Security request
```

Experiment Runner and repository code never call the plugin. Checkout-local
preparation state prevents accidental duplicate preparation in one checkout but
is not global authority and cannot prove cross-machine consumption. Timeout,
safety block, or incomplete output consumes the operational request; every
additional request requires a fresh exact-material operator approval.
