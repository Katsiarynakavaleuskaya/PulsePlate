...
| `.github/workflows/ci-metrics.yml` (`CI Metrics`) | Scheduled / manual CI observability lane | Soft gate | Advisory artifact + `GITHUB_STEP_SUMMARY` reporting only; outside ordinary PR merge truth and must not become a default required PR check | PR4 advisory metrics lane |
...
- PR4 adds `ci-metrics.yml` as an advisory observability lane only. Its outputs are limited to artifacts plus `GITHUB_STEP_SUMMARY`, and it must not be treated as canonical merge truth for ordinary PRs.
