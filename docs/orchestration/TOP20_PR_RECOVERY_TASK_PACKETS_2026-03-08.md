...
| 20 | `PR #1189 (docs/b3-storekit-operational-sync; merge pending)` | 2 | release | `app-store-release-agent` | `marketing-strategist` | `qa-engineer-agent` | Canonical StoreKit product contract and setup checklist are versioned once PR #1189 merges |
...
3. `PR #1189 (docs/b3-storekit-operational-sync; merge pending)` may run in parallel with `PR-TBD-IOS-SUBSCRIPTION-MANAGER` only after the billing baseline contract is runtime-backed.
