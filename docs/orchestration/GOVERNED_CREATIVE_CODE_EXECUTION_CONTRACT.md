...
(`time-second=60`) fail closed. Accepted offsets are normalized to `Z`;
fractional seconds drop insignificant trailing zeros and are omitted when all
digits are zero. Clocks and filesystem times are never inputs. `produced_at` is
deliberately excluded from bundle identity, event fingerprints, idempotency
keys, and event ids. Changing its canonical value still changes canonical
bytes, so replay against an existing sidecar is `divergent_replay`.
