...
- Base: `main`
- Current opening head: `ecb2983751c1c2029acf2bd4a24c9899a082f88d`
...
- [ ] Post-open bot/human review pass completed.
- [ ] No actionable bot comments remain unmapped in `Fixed in Commit Mapping`.

## Premortem Risk Fix Matrix
| Risk ID | Failure mode | Fix | Evidence | Disposition |
| --- | --- | --- | --- | --- |
| PM-DEPS-001 | Source constraints and compiled locks drift. | Updated RAG/vector `.in` and `.txt` direct pins together. | Focused supply-chain tests and source/lock diff audit. | FIXED |
| PM-DEPS-002 | Private Python index cannot serve the new pinned wheel. | Verified direct private-index wheel downloads for both pins. | Private-index `pip download --only-binary=:all: --no-deps` probe. | FIXED |
| PM-DEPS-003 | Emergency wheel manifest becomes stale for newly pinned versions. | Updated exact wheel URLs, SHA256 digests, and `expires_at=2026-06-08`. | `tests/test_install_locked_python_requirements.py`. | FIXED |
| PM-DEPS-004 | PR silently widens profile surface. | Limited diff to RAG/vector profiles, manifest, ledger, and hook-generated baseline. | `git diff --name-status origin/main..HEAD`. | FIXED |
| PM-DEPS-005 | Local `.venv` and CI diverge. | Ran repo `.venv` focused tests and installer preflight; documented heavy local ML limits. | `.venv/bin/python -m pytest -q ...`. | FIXED |
...
| PM-DEPS-007 | Dependabot PR conflicts after another dependency PR lands. | Rebased human branch onto `origin/main` after #1824. | Head `ecb2983751c1c2029acf2bd4a24c9899a082f88d` over base `fb754a1e5c4be687a3828506218993154e076793`. | FIXED |
| PM-DEPS-008 | Unsafe package pin appears unintentionally. | No `pip==` unsafe pin introduced in RAG/vector locks. | `rg -n "^pip==" requirements-rag-vector*.txt scripts/ci/emergency_python_wheels.json`. | FIXED |
| PM-DEPS-009 | Full `make verify` deferred without sufficient bounded evidence. | Used operator-approved machine-heavy exception with focused gates, pre-commit, Experiment Runner, and CI block. | `pre-commit run --all-files`; current-head CI required before merge. | FIXED |
