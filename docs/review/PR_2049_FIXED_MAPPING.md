...
Note: the three pytest commands below were executed with a developer-local
Python interpreter path. The interpreter path is intentionally redacted as
`<redacted-developer-local-python>` to preserve review-evidence portability
without rewriting the command as repo-relative execution.

- PASS: `env PYTHONDONTWRITEBYTECODE=1 <redacted-developer-local-python> -m pytest -q tests/test_creative_code_pr_promotion.py tests/test_creative_code_telemetry.py::test_telemetry_import_does_not_load_promotion_runtime_module -p no:cacheprovider`
- PASS: `env PYTHONDONTWRITEBYTECODE=1 <redacted-developer-local-python> -m pytest -q tests/test_creative_code_telemetry.py tests/test_creative_code_pr_promotion.py -p no:cacheprovider`
- PASS: `env PYTHONDONTWRITEBYTECODE=1 <redacted-developer-local-python> -m pytest -q tests/guards/test_subprocess_uses_absolute_binaries.py tests/guards/test_nosec_policy_guard.py`
