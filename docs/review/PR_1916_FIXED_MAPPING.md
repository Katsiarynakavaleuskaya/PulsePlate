...
| `ATT-RED-015` | Current-head `Pygments exception seam guard` failed because the CI token received HTTP 401 from Dependabot alerts API. | Treat Dependabot alerts 401 the same as 403 by falling back to the public GHSA advisory while keeping non-auth API failures fail-closed. | `test_main_falls_back_to_public_advisory_when_dependabot_alerts_return_401`. | focused Pygments pytest; `pre-commit run --all-files` | pending latest commit | `scripts/ci/check_pygments_exception_guard.py`; `tests/test_check_pygments_exception_guard.py` | FIXED |
