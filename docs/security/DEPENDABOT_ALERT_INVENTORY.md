...
| #224 | GHSA-98m9-hrrm-r99r / CVE-2026-54297 | faraday | ios/Gemfile.lock | runtime | iOS Fastlane release tooling | Release tooling graph, not application runtime | 1.10.6 and 2.14.3 per advisory text; Trivy v0.71.2 still reports >= 2.14.3 | Faraday 1.10.6 resolves inside the current Fastlane graph; Trivy/ruby-advisory-db scanner lag still reports the remediated 1.10.6 lock | codex/dependency-cleanup-faraday-runtime-drift | REMEDIATED to faraday 1.10.6 with temporary exact scanner-lag suppression | docs/security/CVE-2026-54297-faraday-fastlane.md, docs/roadmap/BACKLOG_LEDGER.md#ledger-p1-remove-trivy-suppression-faraday-cve-2026-54297 | 2026-07-04 |
...
2. The `requirements-ci-lite.txt` msgpack alert #225 is a recheck lane only
   ...
   ci-lite path.
3. Faraday is remediated to `1.10.6` in the Fastlane release-tooling lock; the
   remaining follow-up is scanner-lag removal once Trivy/Ruby Advisory DB or
   GitHub Code Scanning stops flagging the remediated lock.
