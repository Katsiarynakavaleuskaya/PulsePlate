...
permanent executable guard is `tests/test_frontend_dependency_guards.py:1538`.
...
The deterministic binding at `tests/test_frontend_dependency_guards.py:2711`
...
`tests/test_frontend_dependency_guards.py:2747` require `4.16.4`, `4.16.5`,
...
The executable guard at `tests/test_frontend_dependency_guards.py:1538`:
...
3. rejects malformed manifest dependency, override, bundled-dependency, and
   workspace containers at `tests/test_frontend_dependency_guards.py:1274`;
4. resolves finite tracked workspace patterns and rejects target-named member
   manifests at `tests/test_frontend_dependency_guards.py:1297`;
...
6. rejects root-lock target demands under either `packages[""]` or
   `packages["."]` at `tests/test_frontend_dependency_guards.py:1579`, plus
...
8. resolves every non-optional demand through the ordered Node ancestor lookup
   candidates defined at `tests/test_frontend_dependency_guards.py:578`, then
...
9. applies npm same-name `optionalDependencies` precedence before selector
   ...
   `tests/test_frontend_dependency_guards.py:3083`;
10. excludes a peer only when `peerDependenciesMeta` contains the exact boolean
   marker `optional: true`;
11. rejects unsupported lock schemas, malformed or prerelease versions,
