...
permanent executable guard is `tests/test_frontend_dependency_guards.py:1572`.
...
The deterministic binding at `tests/test_frontend_dependency_guards.py:2746`
...
`tests/test_frontend_dependency_guards.py:2802` require `4.16.4`, `4.16.5`,
...
The executable guard at `tests/test_frontend_dependency_guards.py:1572`:
...
3. rejects malformed manifest dependency, override, bundled-dependency, and
   workspace containers at `tests/test_frontend_dependency_guards.py:1305`;
4. resolves finite tracked workspace patterns and rejects target-named member
   manifests at `tests/test_frontend_dependency_guards.py:1328`; workspace
   membership is limited to the closed literal or terminal-`/*` recognizer at
   `tests/test_frontend_dependency_guards.py:1289`, while recursive or other
...
6. rejects root-lock target demands under either `packages[""]` or
   `packages["."]` at `tests/test_frontend_dependency_guards.py:1613`, plus
...
8. resolves every non-optional demand through the ordered Node ancestor lookup
   candidates defined at `tests/test_frontend_dependency_guards.py:583`, then
...
9. applies npm same-name `optionalDependencies` precedence before selector
   ...
   `tests/test_frontend_dependency_guards.py:3157`;
...
11. rejects symbolic-link target records at
   `tests/test_frontend_dependency_guards.py:1646`, unsupported lock schemas,
