...
The canonical private proxy served exact compatible Click `8.3.3` and Pillow
`12.3.0` distributions. Lock regeneration used repository Python `3.13.6`,
`pip-tools 7.5.3`, existing output files as seeds, and only targeted
`--upgrade-package click==8.3.3` / `--upgrade-package pillow==12.3.0` requests.
The shared runtime and Docker-runtime locks were regenerated by `pip-compile`.
After the private proxy stalled during the next seeded resolution, the remaining
locks received only the already-resolved exact pins and provenance annotations;
branch-diff evidence verifies that no unrelated package moved and no `pip==`
unsafe stanza appeared, while deterministic guards enforce the resulting floors
and exact Click/Pillow pins across the declared lock surfaces.
