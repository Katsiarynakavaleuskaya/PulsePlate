...
- Evidence:
  - `worker.js:1` defines `/api/*`-only path gating, `GET/POST/OPTIONS`-only method scope, explicit `TARGET_BASE` validation, trusted-origin enforcement via `WORKER_ALLOWED_ORIGINS`, bounded forwarded headers, and `redirect: "manual"`.
