...
```bash
# Генерация ключа (ОБЯЗАТЕЛЬНО: используйте сильный passphrase)
...
# Для CI/CD: настройте ssh-agent или используйте короткоживущие (ephemeral) ключи для автоматизации
```

**🔒 Безопасность:**

1. **Passphrase:** Всегда используйте passphrase при генерации для защиты от компрометации приватного ключа. **Никогда не храните незащищённые (без passphrase) приватные ключи в GitHub Secrets.** Для CI/CD используйте `ssh-agent` (и `SSH_ASKPASS`/Keychain) или переходите на короткоживущие (ephemeral) ключи/менеджер секретов.
