...
- **Go/No‑Go и правила отката**:
  - **Go**: оба целевых порога достигнуты (CTR_lift ≥ 20%, AnomalyRate_reduction ≥ 40%), 95% CI для разницы не включает 0, p‑value < 0.05, guardrails соблюдены → масштабируем Treatment и переходим к Phase 2.
...
- Объясняет рекомендации с уверенностью
