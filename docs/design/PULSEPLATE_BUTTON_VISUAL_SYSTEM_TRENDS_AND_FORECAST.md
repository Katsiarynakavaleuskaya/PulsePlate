...
| `W_HOME_GUIDED_PLANNING_ACTIONS` | Home Guided Planning → Next action | Information-only supporting action after the planning result |
...
| Web | Home | `web.home.open_pro` | `W_HOME_GUIDED_PLANNING_ACTIONS` | Open the information-only Apple product handoff | Solid informational CTA | Soft-glass informational CTA | Precision outline informational CTA | V3 | Information-only states | Text-first; no Store or availability badge | `SORA_BTN_web_home_open_pro_V3_default_V1` + information-only state contract | `PP/Web/Home/GuidedPlanning/AppleProductInfo/Button/Default (TBD)` | Guided Planning → Next action; label is `Learn about PulsePlate for Apple devices`; destination is `/marketing` |
