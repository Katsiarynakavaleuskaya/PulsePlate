...
| Web | Home | `web.home.apple_product_info` | Learn about PulsePlate for Apple devices | `Link` | `frontend/src/pages/Home.tsx:487` -> `/marketing` | Public marketing information page | No API call, payment helper, or entitlement action | No route auth gate | Implemented | None inside the information-only handoff | Keep the destination informational; a paid Web action requires separate exact admission | Route and no-`/pro` acquisition-event test: `frontend/src/pages/__tests__/Home.test.tsx` | `PP/Web/Home/GuidedPlanning/AppleProductInfo/Button/Default (TBD)` | `stub://cta/secondary/apple-product-info` |
...
| Web | Plate | `web.plate.apple_product_info` | Learn about PulsePlate for Apple devices | `button` | `frontend/src/components/PremiumGate.tsx:53` (`setOpen(true)`) | Opens the shared `AppleProductInfoDialog` (`frontend/src/components/PremiumGate.tsx:64`) | No API, payment, entitlement, or acquisition telemetry dependency | Rendered only when `isPremium=false` | Implemented | None inside the bounded information action | Preserve `/bmi`, `/marketing`, and dismissal only; future Web payment needs separate exact admission | Dialog, keyboard, focus-return, and no-acquisition tests: `frontend/src/components/__tests__/PremiumGate.test.tsx` | `PP/Web/Plate/PremiumGate/AppleProductInfo/Button/Default (TBD)` | `stub://cta/information/apple-product` |
...
| iOS | Home | `ios.home.weekly_plan_reader` | Weekly Plan Reader | `NavigationLink` | `ios/PulsePlate/Views/HomeView.swift:112` -> `makeWeeklyPlanReaderScreen()` | Weekly plan reader screen | `/api/v1/pro/meal/weekly` via VM default path (`ios/PulsePlate/ViewModels/WeeklyPlanReaderViewModel.swift:33`) | `FeatureFlags.weeklyPlanReaderEnabled` (`ios/PulsePlate/Utilities/FeatureFlags.swift:47`) | Blocked by flag | Runtime screen is mounted from Home → Pro Tools, but release rollout stays disabled by default; toolbar share and VIP CTA follow-up actions still remain TODO (`ios/PulsePlate/Views/WeeklyPlan/WeeklyPlanReaderView.swift:22`, `ios/PulsePlate/Views/WeeklyPlan/WeeklyPlanReaderView.swift:147`, `ios/PulsePlate/Views/WeeklyPlan/WeeklyPlanReaderView.swift:163`) | Enable controlled release flag path, add Home navigation smoke coverage, complete share/VIP wiring | VM auth/error tests: `ios/PulsePlateTests/WeeklyPlanReaderViewModelTests.swift:26` | `PP/iOS/Home/ProTools/WeeklyPlanReader/Row/Flagged (TBD)` | `stub://cta/pro-tool/weekly-plan` |
| iOS | Home | `ios.home.shopping_list_generator` | Shopping List Generator | `NavigationLink` | `ios/PulsePlate/Views/HomeView.swift:123` -> `makeShoppingListScreen()` | Shopping list screen | `/api/v1/pro/meal/shopping-list` via VM (`ios/PulsePlate/ViewModels/ShoppingListReaderViewModel.swift:37`) | Same flag gate as weekly plan (`ios/PulsePlate/Utilities/FeatureFlags.swift:47`) | Blocked by flag | Runtime screen is mounted from Home → Pro Tools, but normal release flow still lacks a canonical source-of-plan handoff: `DEBUG` bootstraps stub `plan_data`, `RELEASE` opens with `nil`, and backend `weekly_plan_id` path still returns 501 (`app/routers/shopping_list_pro.py:70`) | Define release source-of-plan contract (`weekly_plan_id` or carried `plan_data`), unblock backend path, then add release criteria + CTA tests | No dedicated CTA-level tests | `PP/iOS/Home/ProTools/ShoppingList/Row/Flagged (TBD)` | `stub://cta/pro-tool/shopping-list` |
...
| `web.home.apple_product_info` | `V3` | `W_HOME_QA_GRID` | `SORA_BTN_web_home_apple_product_info_<variant>_<state>_V1` | `docs/design/PULSEPLATE_BUTTON_VISUAL_SYSTEM_TRENDS_AND_FORECAST.md` |
...
| `web.plate.apple_product_info` | `V2` | `W_PLATE_GATE_ACTIONS` | `SORA_BTN_web_plate_apple_product_info_<variant>_<state>_V1` | `docs/design/PULSEPLATE_BUTTON_VISUAL_SYSTEM_TRENDS_AND_FORECAST.md` |
